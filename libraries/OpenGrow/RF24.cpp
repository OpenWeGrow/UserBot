/*
 ...
 Edited @2017 by Open Grow - GroLab, Author: JMelo <joao.melo@opengrow.pt>
...
#ifdef DEBUGRF
unsigned char RFi=0; //Used for iterations
#endif
unsigned char payloadSize = 32;
...
RF24::RF24(uint8_t _cepin, uint8_t _cspin):
  ...
  payload_size(payloadSize), ack_payload_available(false), dynamic_payloads_enabled(false),
...
void RF24::printDetails(void)
{
	...
	Serial.print("AW: ");
	Serial.println(read_register(SETUP_AW),HEX);
	
	Serial.print("RT: ");
	Serial.println(read_register(SETUP_RETR),HEX);
	
	Serial.print("CH: ");
	Serial.println(read_register(RF_CH),HEX);
	...
	Serial.print("RX_ADDR_P2-5: 0x");
	Serial.print(read_register(RX_ADDR_P2),HEX);	
	Serial.print(" - 0x");
	Serial.print(read_register(RX_ADDR_P3),HEX);
	Serial.print(" - 0x");
...
void RF24::begin(void)
{
  ...
  // hardware.
  setDataRate( /*RF24_1MBPS*/RF24_250KBPS) ;

  // Initialize CRC and request 2-byte (16bit) CRC
  setCRCLength( RF24_CRC_16 ) ;
  
  // Disable dynamic payloads, to match dynamic_payloads_enabled setting
  write_register(DYNPD,DISABLE_DYN_PAYLOAD);
  ...
  // spectrum.
    #ifdef SPREAD_RF_CHANNELS 
        setChannel((uint8_t)(((newRFType-1)*5) + (25*(masterID-1))));
    #else
...
void RF24::openWritingPipe(uint64_t value)
{
  ...
  {
    #ifdef SPREAD_RF_CHANNELS 
        setChannel((uint8_t)((newRFType-1)*5) + (25*(masterID-1)) + (value & 0x0F));
    #else
        setChannel((uint8_t)((botType-1)*5) + (25*(masterID-1)) + (value & 0x0F));        
    #endif

        //Serial.println("1!!");
	//Serial.println((uint8_t)((uint8_t)((((botType-1)*5) + (25*(masterID-1))) + value & 0x0F)),DEC);
  }
  else
  {
    #ifdef SPREAD_RF_CHANNELS 
   ...
   Serial.print("V:");
   Serial.println((uint8_t)(value & 0x0F),DEC);*/
...
void RF24::openReadingPipe(uint8_t child, uint64_t address)
{
  ...
  // startListening() will have to restore it.
    ...
    #ifdef SPREAD_RF_CHANNELS 
        setChannel((uint8_t)((newRFType-1)*5) + (25*(masterID-1)) + (address & 0x0F));
    #else
        setChannel((uint8_t)((botType-1)*5) + (25*(masterID-1)) + (address & 0x0F));        
    #endif
	//Serial.println("!!");
        //Serial.println((uint8_t)((uint8_t)((((botType-1)*5) + (25*(masterID-1))) + address & 0x0F)),DEC);
  }
  else
  {
    #ifdef SPREAD_RF_CHANNELS 
  ...
  if (child == 0)
   ...
   //Set a diferent pipe on add 1 for each bot
    ...
    tempPipe[3] = serialNumber[4];
    tempPipe[4] = serialNumber[3];

    write_register(RX_ADDR_P1, tempPipe, 5);

    write_register(RX_ADDR_P2,(serialNumber[7]+1));
    write_register(RX_ADDR_P3,(serialNumber[7]+2));
    write_register(RX_ADDR_P4,(serialNumber[7]+3));
    write_register(RX_ADDR_P5,(serialNumber[7]+4));
...
bool RF24::checkArray(unsigned char * array, unsigned char value)
{
    unsigned char i = 0;
